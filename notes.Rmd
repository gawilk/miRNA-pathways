...
To begin the vignette, run R script 1_CreateDataObjects.R using your working paths to combine raw data files into R data objects and save them. Save them using your preferred naming conventions. Just remember to update variable names in all other scripts to be consistent!

```{bash createData, eval=F, echo=T}
R CMD BATCH scripts/1_CreateDataObjects.R
```

Then run R script 2_NormalizeData.R to filter out samples that are not tumor or normal tissue and convert the RNASeq data into Transcripts per Million (TPM).

```{bash NormalizeData, eval=F, echo=T}
R CMD BATCH scripts/2_NormalizeData.R
```

For the sake of convenience, we have included a Breast Cancer (BRCA) processed ExpressionSet after running 2_NormalizeData.R.  

```{bash BreastData, eval=F, echo=T}
data/BRCAEsetTPM.RData
```

The data in this Breast cancer ExpressionSet looks something like this: 
...
In step 3, we run 3_ReducePathways.R on BRCAEsetTPM.RData, the provided ExpressionSet. This script systematically subsets all gene expression data by pathway genes from the KEGG database. For each pathway, it computes the Isomap embedding, the Pathway Activity Summary (PAS). The meat of the script is in:
...
After having run this analysis on Breast cancer, we now have all the data to plot a figure in the manuscript:
